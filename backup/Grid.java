...
public class Grid extends Cell {
    ...
    public Grid(Position position, int width, int height, int gridWidth, int gridHeight) {
    ...
    public void updateValidMoves(int playerID) {
        ...
        for(Position validMove : validMoves) {
            grid[validMove.x][validMove.y].setValidMove(false);;
        }
        validMoves.clear();
        ...
        // Visually update all valid move positions to show with a valid move highlight
        for(Position validMove : validMoves) {
            grid[validMove.x][validMove.y].setValidMove(true);
        }
    }

    // Checks for all the new positions that would be changed if a move was played at the given position. 
    // Will be empty if the move is invalid.
    public List<Position> getChangedPositionsForMove(Position position, int playerID) {
        List<Position> result = new ArrayList<>();
        result.addAll(getChangedPositionsForMoveInDirection(position, playerID, new Position(1,0))); // Right
        result.addAll(getChangedPositionsForMoveInDirection(position, playerID, new Position(0,1))); // Down
        result.addAll(getChangedPositionsForMoveInDirection(position, playerID, new Position(-1,0))); // Left
        result.addAll(getChangedPositionsForMoveInDirection(position, playerID, new Position(0,-1)));  // Up
        return result;
    }
